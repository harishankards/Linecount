         ...
         <tr align="center">
          ...
          <?php
			...
			elseif(isset($_POST['empname']))
			{
				...
				$query="SELECT * FROM `employee` WHERE `Emp_name` LIKE '%$emp_name%' order by `Emp_no`";
			}
			else
			{
			...
			if($count>0)
			{
				...
				Print "<script type=\"text/javascript\" language=\"javascript\">
						function update(emp_no)
						{
							...
							{
								window.location = \"updateemp.php?no=\"+eno+\"&acc=idsil\";
							}
							else
							{
								return false;
						...
						function deleteemp(emp_no)
						{
							...
							{
								window.location = \"viewemp.php?del_emp_no=\"+eno;
							}
							else
							{
								return false;
			...
			if(isset($_POST['empno']))
			{
				...
				$upsql=mysql_query("UPDATE `employee` SET `Log_status`='$status' WHERE `Emp_no`='$no'");
				if($upsql)
				{
			...
			{		
				...
				if($del)
				{
					...
					echo "<script> dhtmlx.alert({
								...
								callback:function(){
								setTimeout(function(){ window.location = \"viewemp.php\";}, 0);
								}
							});
							</script>";
				}
...
<script>
  $(function() {
    var availableTags = [
		<?php
		$empsql=mysql_query("SELECT `Emp_name` FROM `employee` order by `Emp_no`");
