...
<div id="outer_wrapper">
  <div id="wrapper">
	...
	?>
          ...
          <td>Employee Name</td>
          <td>:</td>
          <td><input type="text" name="e_name" id="e_name" size="35" value="<?php echo $row['Name'];?>"  /> </td><td>( Employee name )</td>
          </tr>
          
          <tr>
          <td>Address</td>
          <td>:</td>
          <td><textarea name="e_add" id="e_add" cols="21" rows="5" ><?php echo $row['Address'];?></textarea></td><td>( Employee Address)</td>
          </tr>
          
          <tr>
          <td>Name of the Bank</td>
          <td>:</td>
          <td><input type="text" name="e_bank_name" id="e_bank_name" size="35" value="<?php echo $row['Bank_name'];?>" /></td><td>( Enter the Bank name ,<br> in which the empployee has account )</td>
          </tr>
          
          <tr>
          <td>Branch Address</td>
          <td>:</td>
          <td><textarea name="e_bank_add" id="e_bank_add" cols="21" rows="5"><?php echo $row['Branch_add'];?></textarea></td><td>( Enter the Branch Address )</td>
          </tr>
          
          <tr>
          <td>IFSC Code</td>
          <td>:</td>
          <td><input type="text" name="e_ifsc" id="e_ifsc" size="35" value="<?php echo $row['IFSC_Code'];?>" /></td><td>( Enter the IFSC Code of the Bank )</td>
          </tr>
          
          <tr>
          <td>Account Type</td>
          <td>:</td>
          <td><input type="text" name="e_acc_type" id="e_acc_type" size="35" value="<?php echo $row['Account_Type'];?>" /></td><td>( Enter the Account Type )</td>
          </tr>
          
          <tr>
          <td>Old Account No</td>
          <td>:</td>
          <td><input type="text" name="e_acc_no" id="e_acc_no" size="35" value="<?php echo $row['Account_No'];?>" readonly="readonly"  /></td><td>( Enter the correct Account Number )</td>
          </tr>
          
           <tr>
          <td>New Account No</td>
          <td>:</td>
          <td><input type="text" name="e_acc_no_new" id="e_acc_no_new" size="35" value="<?php echo $row['Account_No'];?>" /></td><td>( Re-enter the correct Account Number )</td>
          </tr>
          
          <tr>
          <td>Confirm Account No</td>
          <td>:</td>
          <td><input type="text" name="e_acc_no_con" id="e_acc_no_con" size="35" value="<?php echo $row['Account_No'];?>" /></td><td>( Re-enter the correct Account Number )</td>
          </tr>
...
if(isset($_POST['e_vndr'],$_POST['e_name'],$_POST['e_add'],$_POST['e_bank_name'],$_POST['e_bank_add'],$_POST['e_ifsc'],$_POST['e_acc_type'],$_POST['e_acc_no'],$_POST['e_acc_no_con'],$_POST['e_acc_no_new']))
{
	$date=date('Y-m-d');
	...
	if($acc_no==$con_acc_no)
	{
		...
		$enrterby=$loginas;
		$time=$datetime;
		$sql=mysql_query("UPDATE `bank_details` SET  `Vendor`='$vendor',`Date_of_entry`='$date',`Name`='$name',`Address`='$address',`Bank_name`='$bank_name',`Branch_add`='$bank_add',`IFSC_Code`='$ifsc',`Account_Type`='$acc_type',`Account_No`='$con_acc_no',`Enteredby`='$enrterby',`Time`='$time' WHERE `Account_No`='$old_no'");
		if($sql)
		{
