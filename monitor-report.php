...
if(isset($_POST['mls_query']))
{
	$mls_sql = stripslashes($_POST['mls_query']);
	$Use_Title = 1;
	...
	HEADER("Expires: 0");
	$mls_result = MYSQL_QUERY($mls_sql)
	...
	IF (ISSET($w) && ($w==1)) 
	{
		 ...
		 IF ($Use_Title == 1)
		 {
			 ECHO("\n\nMLS UPLOADED File Details\n\n");
		 }
		
		 $sep = "\n";
	 
		 WHILE($row = MYSQL_FETCH_ROW($mls_result))
		 {
			 
			 $schema_insert = "";
			 FOR($j=0; $j<mysql_num_fields($mls_result);$j++)
			 {
			 
			 $field_name = MYSQL_FIELD_NAME($mls_result,$j);
	...
	ELSE
	{
		 ...
		 FOR ($i = 0; $i < (MYSQL_NUM_FIELDS($mls_result)); $i++)
		 {
			 ECHO MYSQL_FIELD_NAME($mls_result,$i) . "\t";
		 }
		 PRINT("\n");
