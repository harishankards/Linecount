...
if(isset($_POST['fileno']))
{
	...
	$result=mysql_query("SELECT * FROM `escript_filedetails` WHERE `File_No` LIKE '%$f_no%'");
...
elseif(isset($_POST['date1'],$_POST['date2'],$_POST['a_id'],$_POST['hos'],$_POST['type'],$_POST['vendor'],$_POST['client'],$_POST['Shift']))
{
	...
	$query="SELECT * FROM `escript_filedetails` FORCE INDEX (`File_No`) WHERE `Date` BETWEEN '$s_date' AND '$e_date'";
...
{	
	...
	$query="SELECT * FROM `escript_filedetails` WHERE `Date` ='$dat'";
	$result=mysql_query($query);
	$ser='';
}
$sendquery=$query;
...
{	
	...
	{	
		...
		echo "<tr class=\"ui-widget-header\"><th >Options<th>S.No.</th><th>Date</th><th>Shift</th><th>Team</th><th>Client</th><th>Hospital</th><th>Platform ID</th><th>File No</th><th>File Type</th><th>LineCount</th><th>ID Status</th><th>MT Name</th><th>Pending Reason</th><th>MT Comment</th><th>QC ID</th><th>QC Name</th><th>QC Comment</th><th>Uploaded To</th><th>Uptime_MLS</th><th>Uptime_Editor</th></tr>";
		...
		echo "<script type=\"text/javascript\" language=\"javascript\">
				...
				function deletefile(file_no)
				{
					...
					{
						window.location = \"viewesupload.php?file_no_del=\"+fno;
					}
					else
					{
						return false;
...
{		
	...
	$del=mysql_query("DELETE FROM `escript_filedetails`  WHERE `File_No`='$value'");
	if($del)
	{
		echo "<script> alert('Details of the File ".$value."  is deleted successfully !!!');</script>";
		echo "<script> window.location = \"viewesupload.php\";</script>";
