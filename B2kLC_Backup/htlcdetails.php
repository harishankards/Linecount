...
<div id="outer_wrapper">
  <div id="wrapper">
	...
	?>
    <div id="main"><br>
        <h2>HT Linecount Details </h2>
        <form name="sub" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
        <center><p class="result">Choose the Date to See InHouse's Line count details</p></center>
...
if(isset($_POST['date1'],$_POST['date2'],$_POST['vendor']))
{
	...
	$col=0;
	//For MLS
	echo" <h2>HT MLS Linecount </h2>";
	echo "<table class=\"text ui-widget-content ui-corner-all tab\" width=\"900\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\">";
	echo "<th>S.No.</th><th>Name</th><th>Blank</th><th>Edit</th><th>DUP Blank</th><th>Dup Edit</th><th>Total LineCount</th><th>Proofing</th><th>Proofing / DUP</th><th>Proofing Total</th><th>Final Total</th>";
	...
	while($row1=mysql_fetch_array($sql))
	{
		...
		if($cl_count!=0)
		{
			...
			{				
				...
				$total_lc=0;
				//$doublepay=0;
				$up_query="SELECT `File_Type`,`Upstatus`,`Shared`,`Linecount` FROM `file_details` WHERE `Date` BETWEEN '$s_date' AND '$e_date' AND `Uploadedby`='$name' AND `Client`='$cl_name'";
				...
				if(($up_count!=0) || ($pr_count!=0) || ($sh_count!=0))
				{
					...
					echo "<tr class=\"tr".$col."\"><td>$a</td><td style=\"text-align:left; padding-left:5px;\">$name ($cl_name)</td><td>$blanklines</td><td>$editlines</td><td>$dup_blank</td><td>$dup_edit</td><td>$total_lc</td><td>$prooflines</td><td>$proofduplines</td><td>$proof_total</td><td>$final</td></tr>";
					$a=$a+1;
					if($col==0)
					{
						$col=1;
	...
	echo "</table><br>";
	//For Editors
	echo "<h2>HT Editors</h2>";
	echo "<table class=\"text ui-widget-content ui-corner-all tab\" width=\"900\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\">";
	echo "<th>S.No</th><th>Name</th><th>Second Level Editing</th><th>Third Level Editing </th><th>DUP Lines</th><th>Missing</th><th> Auditing </th><th> Total </th>";
	$c=1;
	$col1=0;
	...
	while($erow=mysql_fetch_array($e_sql))
	{
		...
		if($cl_count!=0)
		{
			...
			{	
				...
				//final total
				$total_editor_lines=$edtr_lines+$audit_lines+$th_edtr_lines+$edtr_dup_lines+$missing_lines;
				...
				if(($ed_count!=0) || ($th_count!=0) || ($au_count!=0) || ($dup_count!=0))	
				{
					...
					print("<tr class=\"tr".$col1."\"><td>$c</td><td style=\"text-align:left; padding-left:5px;\"> $name ($cl_name) </td><td>$edtr_lines </td><td> $th_edtr_lines </td><td> $edtr_dup_lines</td><td>$missing_lines</td><td> $audit_lines </td><td> $total_editor_lines </td></tr>");
					$c=$c+1;
					if($col1==0)
					{
						$col1=1;
	...
	echo "</table><br>";
	$final_total_lines=$final_blank_lines+$final_edit_lines+$final_dupblank_lines+$final_dupedit_lines+$final_dup_edtr;
	...
	echo "<tr class=\"tr0\"><td style=\"text-align:left; padding-left:5px;\">Total No. of DUP lines by Editor </td><td>$final_dup_edtr</td></tr>";
	...
	Print "<tr>";
	Print "<td>";
	Print "<center><form name=\"xl\" method=\"post\" action=\"htxl.php\">";
	Print "<input type=\"hidden\" value=$s_date name=\"start_ht\" id=\"start_ht\" />";
	Print "<input type=\"hidden\" value=$e_date name=\"end_ht\" id=\"end_ht\" />";
	Print "<input type=\"hidden\" value=$vendor name=\"vendor_ht\" id=\"vendor_ht\" />";
	Print "<input type=\"submit\" value=\"Export to Excel\" name=\"ht\"/>";
