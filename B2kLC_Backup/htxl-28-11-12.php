...
if(isset($_POST['start_ht'],$_POST['end_ht']))
{
	...
	print("\r ");
		$sql=mysql_query("select `Emp_no`,`Emp_name` from `employee` WHERE `Emp_desig`='HT-MLS' order by `Emp_no`");
	...
	print("S.No \t Name \t DUP Lines \t Non DSP \t Second Level Editing \t Third Level Editing \t Auditing \t Total \t  \t  \t  \t \r");
	$c=1;
	$tc=0;
	$e_sql=mysql_query("select `Emp_no`,`Emp_name` from `employee` WHERE `Emp_desig`='HT-EDITOR' order by `Emp_no`");
	while($erow=mysql_fetch_array($e_sql))
	{
		...
		if($cl_count!=0)
		{
			...
			{	
				...
				//For DUP Lines
				$dup_query=mysql_query("SELECT `Linecount`,`Upstatus` FROM `file_details` WHERE `Date` BETWEEN '$s_date' AND '$e_date' AND `Uploadedby`='$name' AND `Client`='$cl_name'");
				...
				//final total
				$total_editor_lines=$edtr_lines+$audit_lines+$th_edtr_lines+$edtr_dup_lines+$edtr_ndup_lines;
				
				if(($ed_count!=0) || ($th_count!=0) || ($au_count!=0) || ($dup_count!=0))	
				{
					print("$c \t $name ($cl_name) \t $edtr_dup_lines \t $edtr_ndup_lines \t $edtr_lines \t $th_edtr_lines \t $audit_lines \t $total_editor_lines \r");
					$c=$c+1;
				}			
			}
		}
