...
if(isset($_POST['date1'],$_POST['date2']))
{
	...
	$comment=$id." has viewed the DUP file details in IDSIL/PJO from ".$s_date." to ".$e_date;
	fwrite($fp, PHP_EOL ."\r\n[".date("d-m-Y:g:i:s A")."] : ".$comment."\n");
	fclose($fp);
	$c=1;
	$r=0;
	$lc=0;
	$result=mysql_query("SELECT `Date`,`Hospital`,`File_No`,`File_Type`,`Linecount` FROM `file_details` FORCE INDEX (`File_No`)  WHERE `Date` BETWEEN '$s_date' AND '$e_date' AND `Uploadedby`='$name'");
